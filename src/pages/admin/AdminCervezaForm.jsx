...
import { getCervezaById, createCerveza, updateCerveza, updateCervezaImatge } from "../../services/cervezasService";

export default function AdminCervezaForm() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (!esEdicion) return;
    getCervezaById(id)
      .then((c) => setForm({ nom: c.nom, tipus: c.tipus, graduacio: c.graduacio, preu: c.preu ?? "", descripcio: c.descripcio || "" }))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (esEdicion) {
        await updateCerveza(id, form);
        if (imagen) {
          const fd = new FormData();
          fd.append("imatge", imagen);
          await updateCervezaImatge(id, fd);
        }
      } else {
        const fd = new FormData();
        Object.entries(form).forEach(([k, v]) => fd.append(k, v));
        if (imagen) fd.append("imatge", imagen);
        await createCerveza(fd);
      }
      navigate("/admin/cervezas");
    } catch (e) {
      setError(e.message);
    } finally {
      setGuardando(false);
  ...
  return (
    <div className="max-w-xl">
      <h2 className="text-xl font-bold text-amber-800 mb-6">
        {esEdicion ? "Editar cerveza" : "Nueva cerveza"}
      </h2>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm p-6 flex flex-col gap-4">
        ...
        <div>
          ...
          <input type="file" accept="image/*" onChange={(e) => setImagen(e.target.files[0])}
            className="w-full text-sm text-gray-500 file:mr-3 file:py-1 file:px-3 file:rounded-lg file:border-0 file:bg-amber-100 file:text-amber-700 hover:file:bg-amber-200" />
        </div>

        <div className="flex gap-3 pt-2">
          <button type="button" onClick={() => navigate("/admin/cervezas")}
            className="flex-1 border border-gray-300 text-gray-600 rounded-lg py-2 text-sm hover:bg-gray-50 transition">
            Cancelar
          </button>
          <button type="submit" disabled={guardando}
            className="flex-1 bg-amber-600 hover:bg-amber-700 text-white font-semibold rounded-lg py-2 text-sm transition disabled:opacity-50">
